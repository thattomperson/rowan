...
module.exports = {
  name: 'evolutions',
  aliases: ['evo'],
  description: 'Find out when a pokemon evolves and any special conditions!',
  usage: '[pokemon name or id]',
	async execute(msg, args) {
    let pokemon
    let chain
    try {
      pokemon = await this.pokedex.getPokemonSpeciesByName(args[0].toLowerCase())
      chain = (await this.pokedex.resource(pokemon.evolution_chain.url)).chain
    } catch (e) {
