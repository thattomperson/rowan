...
const debug = require('debug')('rowan:cmd:evo');
...
async function evolutions(msg, args) {
  let pokemon
  let chain
  try {
    pokemon = await pokedex.getPokemonSpeciesByName(args[0].toLowerCase())
    chain = (await pokedex.resource(pokemon.evolution_chain.url)).chain
  } catch (e) {
